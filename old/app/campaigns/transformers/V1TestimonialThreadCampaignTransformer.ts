...
* @Last Modified time: 2017-07-03 09:33:10
*/
...
namespace lt {
	export class V1TestimonialThreadCampaignTransformer extends CampaignTransformer {
		...
		setIntroPage(campaign: Campaign, form: CampaignForm) {
			return this.getBrandColor(campaign, form).then((color) => {
				...
				if (media.type == "image") {
					...
					media.file_id = form.getFieldValue("image") !== null ? form.getFieldValue("image").id : null;
					media.file = form.getFieldValue("image") !== null ? form.getFieldValue("image") : null;
				} else if (media.type == "video") {
					if (!(media instanceof VideoSection)) {
						media = this.SectionFactory.create(media);
					}
					media.video_id = form.getFieldValue("video") !== null ? form.getFieldValue("video").id : null;
					media.video = form.getFieldValue("video") !== null ? form.getFieldValue("video") : null;
				} else if (media.type == "pdf") {
					if (!(media instanceof PdfSection)) {
						media = this.SectionFactory.create(media);
					}
					media.file_id = form.getFieldValue("pdf") !== null ? form.getFieldValue("pdf").id : null;
					media.file = form.getFieldValue("pdf") !== null ? form.getFieldValue("pdf") : null;
				}
		...
		setThankYouPage(campaign: Campaign, form: CampaignForm) {
		...
		setIntroPageFormFields(form: CampaignForm, campaign: Campaign) {
			var introPage = campaign.site.getIntroPage();
			var introBody = introPage.getBodySection();
			var introMedia = introPage.getMediaSection();

			form.setFieldValue("message", introBody.text !== undefined ? introBody.text : form.getFieldValue("message"));
			...
			if (introMedia instanceof ImageSection) {
				form.setFieldValue("image", introMedia.file ? introMedia.file.valueOf() : null);
			} else if (introMedia instanceof VideoSection) {
				form.setFieldValue("video", introMedia.video ? introMedia.video.valueOf() : null);
			} else if (introMedia instanceof PdfSection) {
				form.setFieldValue("pdf", introMedia.file ? introMedia.file.valueOf() : null);
			}
	...
	angular.module("lt.app").service("V1TestimonialThreadCampaignTransformer", V1TestimonialThreadCampaignTransformer)
}