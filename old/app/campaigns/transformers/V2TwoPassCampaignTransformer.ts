...
*/
namespace lt {
    export abstract class V2TwoPassCampaignTransformer extends CampaignTransformer {
        abstract setShareSections(campaign: Campaign, form: CampaignForm, page: Page, order: number);
        abstract getIntroPageTitleIcon(): string;
        ...
        buildPages(campaign: Campaign, form: CampaignForm) {
            var promise = [];
        ...
        setIntroPageFormFields(form: ICampaignForm, campaign: Campaign): void {
        ...
        setIntroPage(campaign: Campaign, form: CampaignForm) {
            return this.getBrandColor(campaign, form).then((color) => {
                ...
                // Body
                page.addSection({
                    name: "description_body", type: "text"
                }, {
                    order: order++,
