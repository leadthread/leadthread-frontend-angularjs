...
namespace lt {
	export class V1TestimonialThreadCampaignForm extends CampaignForm {
		...
		build(): ICampaignFormForm {
			...
			form["Start"] = <ICampaignFormGroup>{
				"Branding": <ICampaignFormSubGroup>{
					questions: this.brandingQuestions,
				}
			};
			form["Sales Rep Experience"] = <ICampaignFormGroup>{
				"Customer Text Message": {
					questions:[{
						name: "uPMessage",
						displayName: "Message from the sales rep to the customer.",
						description: "This is your text message to your customer, inviting them to share referrals. A link will be added to help them start inviting.",
						value: null,
						type: "textarea",
						limitTo: 280,
						placeholders: false,
						...
						required: true,
						valid: function () {
							return _.isString(this.value) && this.value.length > 20;
						}
					}, {
						name:"pRMessage",
			...
			form["Customer Experience"] = <ICampaignFormGroup>{
				"Intro Page": {
					questions: [{
						name:"message",
			...
			form["Referral Experience"] = <ICampaignFormGroup>{
				"Action Page": {
					questions: [{
						name:"landingText",
	...
	angular.module("lt.app").service("V1TestimonialThreadCampaignForm", V1TestimonialThreadCampaignForm);
}
