angular.module("lt.app").run(["$transitions", "$company", "$favicon", "$state", "$auth", "$localStorage", "$notification", "$app", "$httpPreload", function ($transitions, $company, $favicon, $state, $auth,$localStorage, $notification, $app, $httpPreload) {
	"use strict";
	...
	$transitions.onBefore({ to: "root.company.**" }, function (trans) {
		...
		return $auth.loadRole(_.cloneDeep(trans.params()).companyId).then(function (role) {
			if (_.isArray(roles)) {
			...
			if (!$auth.isLoggedIn() || !isAuthorized) {
				if (trans.to().name !== "root.company.404") {
					$localStorage.set("state-login-target-name", trans.to().name);
					$localStorage.set("state-login-target-params", _.cloneDeep(trans.params()));
				}

				if (!$auth.isLoggedIn()) {
					$notification.error("You are not logged in.");
				} else {
					$notification.error("You are not authorized to view that page.");
				}
