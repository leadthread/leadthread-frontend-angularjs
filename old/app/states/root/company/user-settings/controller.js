...
angular.module("lt.app").controller("UserSettingsController", ["$scope", "$auth", "$api", "$notification", function ($scope, $auth, $api, $notification) {
	"use strict";
	/** 
	...
	function defineScope () {
		...
		if ($scope.user.file_id) {
			$api.show("files", $scope.user.file_id).exec().then(function (res) {
	...
	function validateUser () {
		...
		if (!$scope.user.first_name) {
			...
			$scope.error = "email";
			res = false;
		} else if (!$scope.user.phone || !/^\+?1?[0-9]{10}$/.test($scope.user.phone)) {
			$scope.error = "phone";
			res = false;
		}
	...
	function save () {
		if ($scope.isValid) {
			...
			return $api.update("users", $scope.user).exec().then(function (resp) {
