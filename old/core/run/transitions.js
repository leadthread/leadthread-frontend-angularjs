(function (angular) {
	...
	angular.module("lt.core").run(["$rootScope", function ($rootScope) {
		$rootScope.$on("$stateChangeError", console.error);
	}]);

	angular.module("lt.core").run(["$transitions", "$auth", "$state", "$localStorage", "$device", "$window", "$location", function ($transitions, $auth, $state, $localStorage, $device, $window, $location) {
