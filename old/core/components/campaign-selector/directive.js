/* global _:false */
angular.module("lt.core").directive("campaignSelector", ["UserService", "$http", "$device", "$color", "$cache", "$auth", "$stateParams", function (UserService, $http, $device, $color, $cache, $auth, $stateParams) { 
	"use strict";
	...
	return {
		...
		link: function ($scope, $el, attrs, $select) {
			...
			function defineScope () {
				...
				$scope.campaigns = _.isArray($scope.campaigns) ? $scope.campaigns : [];
				$scope.search = search;
			...
			function search (camp) {
				...
				if (camp.name.toLowerCase().indexOf($select.search || "") !== -1) {
					return true;
				}
