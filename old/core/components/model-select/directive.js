(function (angular, _) {
	...
	angular.module("lt.core").directive("modelSelect", ["$api", "$q", "$window", function ($api, $q, $window) { 
		return {
			restrict: "A",
			templateUrl: "components/model-select/index.html",
			scope : {
				"path": "@",
				"models": "=",
				"selected": "=",
			...
			link: function ($scope) {
				...
				function defineScope () {
					...
					$scope.displayColumn = $scope.displayColumn ? $scope.displayColumn : "id";

					//Select a model
					$scope.selected = (_.isObject($scope.selected) ? $scope.selected : selectFirst());
				...
				function selectFn (obj) {
					return $q(function (resolve) {
						$scope.options.actions.select.doSelect(obj).then(function (obj) {
							resolve(obj);
				...
				function newFn () {
					return $q(function (resolve) {
						$scope.options.actions.new.doNew()
						.then(function (obj) {
				...
				function saveFn (obj) {
					return $scope.options.actions.save.doSave(obj).then($scope.options.actions.save.onSave, onError);
				}
				function deleteFn (id) {
					return $scope.options.actions.delete.doDelete(id)
						.then(function () {
				...
				function doSave (obj) {
					return $api.update($scope.path, obj).exec().then(function (x) {return x.data;});
				}
				function doDelete (id) {
					return $api.destroy($scope.path, id).exec().then(function (x) {return x.data;});
				}
