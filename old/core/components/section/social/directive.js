(function (angular, a2a, _) {
	...
	angular.module("lt.core").directive("sectionSocial", ["$q", "$device", "$timeout", "$popup", "$http", "$notification", function ($q, $device, $timeout, $popup, $http, $notification) { 
		return {
			restrict: "A",
			...
			link: function ($scope, $el) {
				function init () {
					...
					// Exec
					getSocialRequirements().then(function (resp) {
					...
					if (typeof a2a !== "undefined") {
						setTimeout(function () {
							function my_addtoany_onshare (share_data) {
								...
								$http.post("/api/v1/links/sent-via", {long_url: share_data.url, sent_via: share_data.node.a2a.safename}).then(function () {
									$scope.lock = "Waiting";
									$q.when($scope.options.getUrlFn({sent_via: null})).then(function (resp) {
				...
				function getSocialRequirements () {
					...
					if (_.isFunction($scope.options.getUrlFn)) {
						preA2APromises.url = $q.when($scope.options.getUrlFn({sent_via: null}));
					}
					if (_.isFunction($scope.options.getSocialMsgFn)) {
						preA2APromises.msg = $q.when($scope.options.getSocialMsgFn());
					}
