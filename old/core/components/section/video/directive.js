(function (angular, _) {
	...
	angular.module("lt.core").directive("sectionVideo", ["$api", "$timeout", "EventService", function ($api, $timeout, EventService) { 
		return {
			restrict: "A",
			...
			link: function ($scope, $el) {
				...
				function init () {
					...
					if ($scope.model.type === "video" && !$scope.model.video.id && $scope.model.id) {
						$api.show("sections", $scope.model.id).index("videos").exec().then(function (resp) {
					...
					$scope.$watch(function () {
						...
						if ($scope.model && $scope.model.video && $scope.model.video.id) {
