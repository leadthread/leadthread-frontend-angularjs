(function (angular, a2a, _) {
	"use strict";

	angular.module("lt.core").directive("sectionTestimonial", ["$q", "$popup", "$window", function ($q, $popup, $window) { 
		return {
			restrict: "AE",
			...
			link: function ($scope, $el) {
					...
					if (story.type === "image") {
						collectCaption().then(function () {
							if (!$scope.options.story.image) {
								$popup.decide("Do you want to add a fun image to attach to your message?", "You can use your phone's camera or select an existing image from your phone.").then(collectImage, nextStep);
							}
						});
					}
				...
				function collectCaption () {
					return $popup.textarea("caption", $scope.options.story.caption, "What do you want to say to your contacts?", null, 6).then(function (resp) {
				...
				function isStoryDone () {
					...
					case "video":
						...
						break;
					case "image":
						if (_.isString(story.caption) && story.caption.length > 5) {
							done = true;
						}
						break;
					}
				...
				function scrollToSelf () {
					setTimeout(function () {
						...
						angular.element("html, body").animate({scrollTop: top}, "fast");
					}, 200);
				}
...
})(window.angular, window.a2a, window._);