angular.module("lt.core").directive("durationPicker", [function () { 
	"use strict";

	return {
		restrict: "EA",
		templateUrl: "components/duration-picker/index.html",
		scope : {
			ngModel: "=",
			as: "=?",
			min: "@?",
		...
		link: function ($scope) {
			...
			function init () { 
				...
				$scope.as = _.indexOf($scope.intervals, $scope.as) >= 0 ? $scope.as : _.first($scope.intervals);
				...
				$scope.$watch("value", function () {
					$scope.ngModel = reduce($scope.value, $scope.interval, $scope.as);
				});
			...
			{
				var x = ["milliseconds", "seconds", "minutes", "hours", "days", "weeks", "months", "years"];
