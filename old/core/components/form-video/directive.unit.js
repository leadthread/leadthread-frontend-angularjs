...
describe("formVideo directive", function () {
	...
	it("constructs directive", inject(function ($httpBackend) {
		$httpBackend.whenPOST("http://localhost:9736/api/v1/videos")
			.respond({data: {}});
		$compile("<div form-video='{video_id:1, video_url:\"https://www.youtube.com/watch?v=5pFX2P7JLwA\", video_service:\"viddler\"}'></div>")($rootScope);
		$rootScope.$apply();
		$rootScope.$digest();
		$httpBackend.flush();
	}));
