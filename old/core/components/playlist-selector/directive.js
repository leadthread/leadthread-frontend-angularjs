/* global _:false */
angular.module("lt.core").directive("playlistSelector", ["$api", "$http", "$device", "$color", "$cache", "$auth", function ($api, $http, $device, $color, $cache, $auth) { 
	"use strict";
	...
	return {
		...
		link: function ($scope, $el, attrs, $select) {
			...
			function defineScope () {
				...
				$scope.playlists = _.isArray($scope.playlists) ? $scope.playlists : [];
