...
describe("Directive \"backButton\"", function () {
	...
	it("can be created", function () {
		...
		expect(element.html()).not.toContain("class=\"icon-chevron-left ng-scope\"></span>");
	});

	it("can be created with the icon", function () {
		// Compile a piece of HTML containing the directive
		var element = $compile("<div back-button=\"Test\" icon=\"true\"></div>")($rootScope);
		$rootScope.$digest();
		expect(element.html()).not.toContain(">Back</span>");
		expect(element.html()).toContain("class=\"icon-chevron-left ng-scope\"></span>");
	});
});