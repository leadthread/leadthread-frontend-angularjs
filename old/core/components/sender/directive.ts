namespace lt {
	...
	interface ISenderScope extends ng.IScope {
		contacts: IContact[];
		options: ISiteOptions;
		type: string;
	}

	angular.module("lt.core").directive("sender", ["$localStorage", "$http", "$auth", "$company", "$q", function ($localStorage: IStorageService, $http: ng.IHttpService, $auth: IAuthService, $company: ICompanyService, $q: ng.IQService) {
		return {
			restrict: "A",
			...
			link: function ($scope: ISenderScope) {
				/**
				 * Start it up
				 */
				function init () {
					...
					var contacts = load();
					$scope.contacts = _.unionWith($scope.contacts, contacts, function (a: IPerson, b: IPerson) {
				...
				function getCampaignLink () {
					return $q(function (resolve, reject) {
						$company.getId().then(function (company_id) {
							return $http.post("/api/v1/links/campaign", {
				 ...
				 */
				function getSmsLink (index: number) {
				...
				function markAsSent (index: number) {
					var contact = getContactsToSend()[index];
					contact.sent = true;
					$http.post("/api/v1/links/sent-via", {long_url: contact.link.url, sent_via: "sms"});
				}
