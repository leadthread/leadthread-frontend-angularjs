(function (angular) {
	...
	angular.module("lt.core").directive("messageHistory", ["$http", "$timeout", "$stateParams", "$messages", function ($http, $timeout, $stateParams, $messages) { 
		return {
			restrict: "EA",
			...
			link: function ($scope) {
				...
				function onContactIdChange (id) {
					$messages.loadMessages(id).then(function (messages) {
						if (id === $scope.contact.id) {
							$scope.contact.messages = _.unionBy($scope.contact.messages, messages, "id");
						}
					});
				}
				...
				var sendMessage = function () {
					var msg = $scope.draft;
					$scope.draft = "";
					$http.post("/api/v1/sms/send/message", {
						contact_id: $scope.contact.id,
						message: msg,
						company_id: $stateParams.companyId
					}).then(function () {
						var timestamp = moment().format("YYYY-MM-DD HH:mm:ss");
						$scope.contact.messages.push({message: msg, inbound: false, seen: true, created_at: timestamp});
