...
describe("Accordion Category directive", function () {
	...
	it("constructs directive", function () {
		$httpBackend.whenGET(/http:\/\/localhost:9736\/api\/v1\/files\/*?/)
			.respond({data: [{
				id: 1,
				fingerprint:"aaaaaaaaaaaaaaaaaa"
			}]});
		$compile("<categories-directive step='1' cat='auto' prompt='\"hello there\"' option='{chosen:[]}'></categories-directive>")($rootScope);
		$rootScope.$apply();
		$rootScope.$digest();
