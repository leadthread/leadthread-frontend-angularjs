(function (angular, _) {
	...
	angular.module("lt.core").directive("navbarDirective", ["$rootScope", "$stateParams", "$state", "$pages", "$auth", "$api", "$http", "$company", "$notification", "$favicon", "$transitions", "$messages", "$title", function ($rootScope, $stateParams, $state, $pages, $auth, $api, $http, $company, $notification, $favicon, $transitions, $messages, $title) {
		return {
			restrict: "EA",
			...
			link: function ($scope) {
				...
				function defineListeners () {
					$transitions.onSuccess({ to: "root.company.**" }, onStateChangeSuccess);
					...
					$scope.$watch(function () {
						...
						if (n !== o) {
							...
							if (company.status == "disabled") {
								$notification.error(company.name + " is disabled. Please contact your Yaptive representative.");
					...
					$scope.$watch(function () {
						return $scope.selectedCompany ? $scope.selectedCompany.id : null;
					}, onCompanyChange);
				...
				function updateFavicon () {
					$company.getFavicon().then(function (favicon) {
						$scope.favicon = "/files/"+favicon.fingerprint+"/stretch/50/50";
					}, function () {
