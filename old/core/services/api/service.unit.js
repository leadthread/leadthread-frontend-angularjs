...
describe("$api", function () {
	...
	describe("requests", function () {
		...
		it("calls index route for model", function () {
			...
			$httpBackend.expectGET("http://localhost:9736/api/v1/users")
				.respond(mockIndexResponse);

			$httpBackend.expectGET("http://localhost:9736/api/v1/users/1+2")
				.respond(mockBatchResponse);

			$api.index("users").exec().then(function (data) {
		...
		it("calls index and only fetches the uncached resources", function () {
			...
			$httpBackend.expectGET("http://localhost:9736/api/v1/users/1")
				.respond(mockFirstShowResponse);
			
			$httpBackend.expectGET("http://localhost:9736/api/v1/users")
				.respond(mockIndexResponse);

			$httpBackend.expectGET("http://localhost:9736/api/v1/users/2")
				.respond(mockSecondShowResponse);

			$api.show("users", 1).exec().then(function (resp) {
				expect(resp.data).toEqual(mockFirstShowResponse);
				$api.index("users").exec().then(function (resp) {
		...
		it("calls nested index route for model", function () {
			...
			$httpBackend.expectGET("http://localhost:9736/api/v1/users/1/referrals")
				.respond(mockIndexResponse);

			$httpBackend.expectGET("http://localhost:9736/api/v1/users/1/referrals/1+2")
				.respond(mockBatchResponse);

			$api.show("users", 1).index("referrals").exec().then(function (data) {
				expect(data).toEqual({data: mockBatchResponse});
			});
		...
		it("returns an array for a batch request with a single id", function () {
			...
			$api.batch("users", [1]).exec().then(function (resp) {
	...
	it("returns an empty array for an empty batch request", function () {
		$api.batch("referrals", []).exec().then(function (data) {
