(function (angular, _, md5) {
	...
	Notification.prototype.notify = function (obj, wait) {
		...
		var duplicate = _.find(_.concat(this.notifications, this.queue), {hash: hash});

		if (!duplicate) {
			if (this.notifications.length < 7) {
				obj.hash = hash;
				obj.timeout = this.$timeout(_.bind(function () {
					this.deNotify(obj);
				}, this), obj.wait);
	...
	Notification.prototype.deNotify = function (obj) {
		if (this.notifications.length > 0) {
			this.$rootScope.$apply(_.bind(function () {
				if (_.get(obj,"hash")) {
	...
	angular.module("lt.core").service("$notification", ["$rootScope", "$timeout", Notification]);
