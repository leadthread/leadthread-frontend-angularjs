angular.module("lt.core").factory("$pages", ["PAGES", "$auth", "$q", "CompanyService", "$app", function (PAGES, $auth, $q, CompanyService, $app) {
	"use strict";
	...
	var checkPagePermissions = function (role, page, count) {
		...
		if (_.isArray(page.pages) > 0 && _.filter(page.pages, {enabled: true}).length > 0) {
			page.enabled = page.enabled || true;
		}

		_.forEach(page.role, function (pageRole) {
			if (_.isEqual(pageRole, role) && (!page.onlySingleCompanies || count > 1)) {
				page.enabled = page.enabled || true;
			} else {
