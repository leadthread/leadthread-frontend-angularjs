...
angular.module("lt.core").factory("$messages", ["$rootScope", "$q", "$api", "$http", "$company", function ($rootScope, $q, $api, $http, $company) {
	"use strict";
	...
	function loadContacts () {
		return $company.getId().then(function (companyId) {
			return $http.get("/api/v1/sms/conversations", {
				params: {
	...
	function loadMessages (contact_id) {
		return getContact(contact_id).then(function (contact) {
			return $api.show("contacts", contact.id).index("messages").exec().then(function (response) {
				contact.messages = _.unionBy(contact.messages, response.data, "id");
	...
	function getContact (contact_id) {
		return $q(function (resolve, reject) {
			...
			if (contact) {
				...
				return $api.show("contacts", contact_id).exec().then(function (resp) {
					var contact = _.find(contacts, function (c) {
	...
	function markMessagesAsRead (contact) {
		_.forEach(contact.messages, function (message) {
			if(!message.seen) {
				$http.put("/api/v1/messages/"+message.id+"/read").then(function () {
					message.seen = true;
				});
			}
		});
