...
describe("$cache Spec", function () {
	...
	it('should properly check if key exists or not', inject(function($q, $rootScope) {
		// Should not exist
	...
	it('should forget cache keys properly when using a partial key', inject(function($q, $rootScope) {
		$cache.remember('value', 10, function () {
			return 1234;
		})
		// Propagate promise resolution to 'then' functions using $apply().
