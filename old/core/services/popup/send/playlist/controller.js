angular.module("lt.core").controller("PopupSendPlaylistController", ["options", "$scope", "$uibModalInstance", "$q", "PlaylistScheduleService", "CampaignService", function (options, $scope, $uibModalInstance, $q, PlaylistScheduleService, CampaignService) {
	function init () {
		defineScope();
		defineListeners();
		compileSchedules();
	...
	function compileSchedules () {
		$scope.loading = true;
		return PlaylistScheduleService.indexFor("playlists", $scope.options.playlist.id).then(function (schedules) {
			...
			return $q.all(campaigns).then(function (campaigns) {
				$scope.options.playlist.schedules = _(schedules)
					...
					.map(function (schedule) {
						schedule.campaign = _.find(campaigns, {id: schedule.campaign_id});
						return schedule;
					})
					.value();
				$scope.loading = false;
	...
	function getNextSchedule () {
		return _($scope.options.playlist.schedules).filter({tense:"future"}).first();
	}

	function getIndexOfNextSchedule () {
		var first =  getNextSchedule();
		return _.findIndex($scope.options.playlist.schedules, {id: first.id});
	}
	...
	function compileScheduleDate (schedule) {
		if ($scope.options.playlist.type === "date") {
			...
			schedule.date = moment(schedule.timer_time, "HH:mm:ssZ").add(schedule.timer_days, "days");
