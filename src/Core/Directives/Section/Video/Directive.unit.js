...
describe("sectionVideo directive", function () {
	...
	it("constructs directive", inject(function ($httpBackend) {
		$httpBackend
			.whenGET("http://localhost:9736/api/v1/sections/1/videos")
			.respond({
				data: [
					{
						id: 1,
		...
		$httpBackend
			.whenGET("http://localhost:9736/api/v1/sections/1/videos/1")
			.respond({
				data: {
					id: 1,
					video_id: 1,
					video_url: "https://www.youtube.com/watch?v=hRAFPdDppzs",
				},
			})

		$compile("<div section-video='{id:1, type:\"video\"}'></div>")(
			$rootScope
		)
		$rootScope.$apply()
		$rootScope.$digest()
