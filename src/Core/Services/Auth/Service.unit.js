...
describe("$auth", function () {
	...
	function login() {
		$httpBackend.expectPOST("/auth/login").respond({ data: { id: 1 } })

		$httpBackend
			.expectGET("http://localhost:9736/api/v1/users/1/companies")
			.respond({ data: [{ id: 1 }] })

		$httpBackend
			.expectGET("http://localhost:9736/api/v1/users/1/companies/1")
			.respond({ data: { id: 1 } })

		$httpBackend
			.expectGET(
				"http://localhost:9736/api/v1/users/1/roles?company_id=1"
			)
			.respond({ data: [{ id: 1 }] })

		$httpBackend
			.expectGET("http://localhost:9736/api/v1/users/1/roles/1")
	...
	it("should fail to login if email and/or password are not strings", inject(function (
		$rootScope
	) {
		var firstError
		var secondError
	...
	it("should login and get localStorage data", inject(function (
		$q,
		$rootScope
	) {
		login()
	...
	it("should login with invite code", inject(function ($rootScope) {
		...
		$httpBackend.expectPOST("/auth/code").respond({
			data: {
				code: { code: code },
				contact: { name: "name", id: 1 },
			},
		})
	...
	it("should logout", inject(function ($q, $rootScope) {
		$httpBackend.whenGET("/auth/logout").respond(true)
		login()
		$httpBackend.flush()
		$rootScope.$apply()
		$auth.logout()
	...
	it("should register", inject(function ($rootScope) {
		$httpBackend.whenPOST("/links/register").respond({ data: "test" })
