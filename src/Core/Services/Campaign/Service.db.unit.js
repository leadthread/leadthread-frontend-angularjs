...
describe("$campaignDb", function () {
	var $campaignDb,
		$httpBackend,
		Campaign,
		CustomerExperienceCampaign,
		CustomerExperienceCampaignSite,
		PageList,
		IntroPage,
		ThankYouPage,
		SectionList,
	...
	var mockCampaignResponse = {
		data: { id: 1, name: "Test Campaign", site_id: 1, type: "referral" },
	}
	var mockSiteResponse = {
		data: { id: 1, name: "Test Site", site_id: 1, type: "referral" },
	}
	var mockPageIndexResponse = { data: [{ id: 1 }, { id: 2 }] }
	var mockPageBatchResponse = {
		data: [
	...
	var mockSectionOneResponse = {
		data: { id: 1, name: "Test Section #1", page_id: 1, type: "text" },
	}
	var mockSectionTwoResponse = {
		data: { id: 2, name: "Test Section #2", page_id: 2, type: "text" },
	}
	...
	beforeEach(function () {
		...
		inject(function ($injector) {
			...
			CustomerExperienceCampaign = $injector.get(
				"CustomerExperienceCampaign"
			)
			CustomerExperienceCampaignSite = $injector.get(
				"CustomerExperienceCampaignSite"
			)
	...
	it("Should save a campaign", inject(function ($q, $rootScope) {
		...
		campaign.load().then(function (campaign) {
			$httpBackend
				.expectPOST("http://localhost:9736/api/v1/sites")
			...
			$httpBackend
				.expectPOST("http://localhost:9736/api/v1/action-pages")
				.respond({ data: campaign.action_page.valueOf() })

			$httpBackend
				.expectPOST("http://localhost:9736/api/v1/campaigns")
	...
	it("Should load a campaign", inject(function ($q, $rootScope) {
		$httpBackend
			.expectGET("http://localhost:9736/api/v1/campaigns/1")
			.respond(mockCampaignResponse)

		$httpBackend
			.expectGET("http://localhost:9736/api/v1/sites/1")
			.respond(mockSiteResponse)

		$httpBackend
			.expectGET("http://localhost:9736/api/v1/sites/1/pages")
			.respond(mockPageIndexResponse)

		$httpBackend
			.expectGET("http://localhost:9736/api/v1/sites/1/pages/1+2")
			.respond(mockPageBatchResponse)

		$httpBackend
			.expectGET("http://localhost:9736/api/v1/pages/1/sections")
			.respond(mockPageOneSectionIndexResponse)

		$httpBackend
			.expectGET("http://localhost:9736/api/v1/pages/2/sections")
			.respond(mockPageTwoSectionIndexResponse)

		$httpBackend
			.expectGET("http://localhost:9736/api/v1/pages/1/sections/1")
			.respond(mockSectionOneResponse)

		$httpBackend
			.expectGET("http://localhost:9736/api/v1/pages/2/sections/2")
		...
		$campaignDb.load(1, "referral").then(function (campaign) {
			...
			expect(campaign.site).toEqual(
				jasmine.any(CustomerExperienceCampaignSite)
			)
			expect(campaign.site.pages).toEqual(jasmine.any(PageList))
			expect(campaign.site.pages[0]).toEqual(jasmine.any(IntroPage))
			expect(campaign.site.pages[0].sections).toEqual(
				jasmine.any(SectionList)
			)
			expect(campaign.site.pages[0].sections[0]).toEqual(
				jasmine.any(TextSection)
			)
			expect(campaign.site.pages[1]).toEqual(jasmine.any(ThankYouPage))
			expect(campaign.site.pages[1].sections).toEqual(
				jasmine.any(SectionList)
			)
			expect(campaign.site.pages[1].sections[0]).toEqual(
				jasmine.any(TextSection)
			)
		})
