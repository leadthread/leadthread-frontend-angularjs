...
import {
	...
	PdfSection,
	ImageSection,
} from "../../../Core/Classes/Models/Section"
	...
	setIntroPage(campaign, form) {
		return this.getBrandColor(campaign, form).then((color) => {
			...
			if (media.type == "image") {
				...
				media.type = "image"
				media.file_id =
					form.getFieldValue("defaultImage") !== null
						? form.getFieldValue("defaultImage").id
						: null
				media.file =
					form.getFieldValue("defaultImage") !== null
						? form.getFieldValue("defaultImage")
						: null
				...
				media.type = "video"
				media.video_id =
					form.getFieldValue("defaultVideo") != null
						? form.getFieldValue("defaultVideo").id
						: null
				media.video =
					form.getFieldValue("defaultVideo") != null
						? form.getFieldValue("defaultVideo")
						: null
				...
				media.type = "pdf"
				media.file_id =
					form.getFieldValue("defaultPDF") != null
						? form.getFieldValue("defaultPDF").id
						: null
				media.file =
					form.getFieldValue("defaultPDF") != null
						? form.getFieldValue("defaultPDF")
						: null
			}
			...
			_.forEach(buttons, (button, i) => {
				...
				if (!s) {
					introPage.deleteSections((section) => {
			...
			introPage.setButtonSection(
				find,
				this.SectionFactory.create(actionButton.valueOf())
			)
...
export const module = {
	key: "V1ReferralThreadCampaignTransformer",
	fn: V1ReferralThreadCampaignTransformer,
}
