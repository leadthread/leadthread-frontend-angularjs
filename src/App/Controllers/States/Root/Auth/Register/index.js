...
const key = "AuthRegisterController"
...
const fn = (
	...
	function init() {
		...
		if ($device.isMobile()) {
			...
			$api.show("links", $stateParams.linkId)
				.exec()
				.then(function (resp) {
					$scope.url = resp.data
				})
	...
	function validate() {
		$scope.canSubmit =
			validateFn($scope.local.fn) &&
			validateCp($scope.local.cp) &&
			validateLn($scope.local.ln) &&
			validatePn($scope.local.pn) &&
			validateEm($scope.local.em)
	}
		...
		if ($scope.canSubmit) {
			$auth
				.register(
					...
					$scope.validation.cp,
					$stateParams.linkId,
					$scope.rep.data.id ? $scope.rep.data.id : undefined
				)
				.then(function (resp) {
