...
const fn = (
	...
	var defineScope = function () {
		...
		playlistView(
			_.find($scope.panels.playlists.items, {
	...
	var playlistInsertIntoArray = function (playlist) {
		$scope.panels.playlists.items = _.unionBy(
			$scope.panels.playlists.items,
			[playlist],
			"id"
		)
		return playlist
	...
	var playlistNew = function () {
		return playlistOpenForm().then(playlistSave).then(playlistView)
	}

	var playlistEdit = function (model) {
		return playlistOpenForm(model).then(playlistSave)
	}
	...
	var playlistDelete = function (playlist) {
		return $popup
			.decide("Are you sure you want to delete this?")
			.then(function () {
				return PlaylistService.destroy(playlist.id).then(function () {
					_.remove($scope.panels.playlists.items, {
						id: playlist.id,
					})
	...
	var contactsAdd = function () {
		$state.go("^.send", {
			type: "playlist",
			playlistId: _.get($scope, "panels.playlists.active.id"),
		})
	}
	...
	var contactDelete = function (playlist, contact) {
		return $popup
			...
			.then(function () {
				return $http
					.post("/api/v1/playlists/" + playlist.id + "/detach", {
						contact_ids: [contact.id],
					})
					.then(function () {
						if (
							playlist.id ===
							_.get($scope, "panels.playlists.active.id")
						) {
							_.remove($scope.panels.contacts.items, {
								id: contact.id,
							})
						}
	...
	var scheduleSetCampaign = function (schedule) {
		schedule._campaign = _.find($scope.campaigns, {
	...
	var scheduleInsertIntoArray = function (schedule) {
		$scope.panels.schedules.items = _.unionBy(
			[schedule],
			$scope.panels.schedules.items,
			"id"
		)
		return schedule
	}
	...
	var scheduleDelete = function (schedule) {
		return $popup
			.decide("Are you sure you want to delete this?")
			.then(function () {
				return PlaylistScheduleService.destroyFor(
					"playlists",
					$scope.panels.playlists.active.id,
					schedule.id
				).then(function () {
					_.remove($scope.panels.schedules.items, {
						id: schedule.id,
					})
				})
			})
	...
	var scheduleSave = function (model) {
		...
		return PlaylistScheduleService.saveFor(
			"playlists",
			$scope.panels.playlists.active.id,
			model
		).then(scheduleInsertIntoArray)
