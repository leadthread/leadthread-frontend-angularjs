...
describe("BuilderController", function () {
	var $company,
		...
		CampaignFactory,
		$auth,
		CustomerExperienceCampaignForm,
		WarmIntroCampaignForm
	...
	beforeEach(inject(function (
		...
		_CampaignFactory_,
		_$auth_,
		_CustomerExperienceCampaignForm_,
		_WarmIntroCampaignForm_,
		_$httpBackend_
	) {
	...
	var setup = function () {
		...
		$httpBackend
			.whenGET("http://localhost:9736/api/v1/companies/1/logo")
			.respond({ data: [{ id: 1 }] })

		$httpBackend
			.whenGET("http://localhost:9736/api/v1/companies/1/logo/1")
			.respond({ data: { id: 1 } })

		$httpBackend
			.whenGET("http://localhost:9736/api/v1/users/1/companies")
			.respond({ data: [{ id: 1 }] })

		$httpBackend
			.whenGET("http://localhost:9736/api/v1/users/1/companies/1")
			.respond({ data: { id: 1 } })
	...
	it("should load up a customer-experience campaign", inject(function (
		$rootScope
	) {
		setup()
		var company = { data: { data: { name: "Blah" } } }
		var $scope = $rootScope.$new()
		spyOn($campaignDb, "load").and.returnValue(
			$q.when(getCampaign("customer-experience"))
		)
		var $stateParams = {
			campaignId: 1,
		...
		var controller = $controller("BuilderController", {
			$scope: $scope,
			company: company,
