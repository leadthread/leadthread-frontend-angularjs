...
const fn = (
		...
		$scope.types = [
			{
				...
				description:
					"The simplest way for your customers, colleagues, and partners to introduce you to their personal contacts by sharing (via SMS or social media) an Action page that promotes your product/service and makes it easy for referrals to contact you.",
			},
			{
				name: "Leaderboard",
				key: "leaderboard",
			...
			},
			{
				...
				description:
					"Need to share important information and set a clear call-to-action? MessageThread delivers your most updated  marketing information, follow up messages, and important updates directly to your target audience via SMS with actionable next steps.",
			},
			{
				...
				description:
					"Customers are invited to share their buying experience via video or written testimonials and share it with all their friends on social media or directly via SMS to expand your reach and warm up potential opportunities.",
			},
			{
				name: "Recognition",
				key: "recognition",
				...
				description:
					"For sending out quick messages and videos to recognize your employees and keep them engaged about things like: Benefits, Company Party, Career Opportunities, Thanks, Awards, etc.",
			},
			{
				name: "Reach",
				key: "reach",
				...
				description:
					"For sending strategic, text based messages with quick 1-2 minute videos directly to employees for: Best	practices, Motivational, Revenue Drivers, Strategic Information for adapting and winning.",
			},
			{
				name: "Review",
				key: "review",
				...
				description:
					"For sending a pre-loaded text message campaigns to employees – who you know will increase your online scores for Glassdoor, Indeed, etc.",
			},
		]
	...
	function isTypeDisabled(type) {
		if (!type.disabled) {
			$state.go("root.company.create.new", {
		...
		$scope.$watch(
			...
			function (n, o) {
				...
				if (n != o && campaign.id && campaign.type) {
					...
					$state.go("root.company.create.edit", {
	...
	function copyCampaign(campaign) {
		$popup
			.copy(
				campaign,
				_.filter(companies, function (company) {
			...
			.then(function (options) {
				return $http
					.post("/api/v1/campaigns/" + campaign.id + "/copy", {
						name: options.name,
